...
{
    public GameObject tb, tbs, tbCards, tbSpin,tbnotainguyen, mode;
    ...
    public void ShowTbsCards()
    {
        if (!isRunningTbs)
        {
            StartCoroutine(ShowWithScaleEffect(tbCards, originalScaleTbs, waitTimeTbs, () => isRunningTbs = false));
            isRunningTbs = true;
        }
    }
    ...
    public void ShowMode2()
    {
        if (!isRunningTbs)
        {
            StartCoroutine(ShowWithScaleEffect(mode, originalScaleTbs, waitTimeTbs, () => isRunningTbs = false));
            isRunningTbs = true;
        }
    }
